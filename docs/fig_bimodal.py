...
pmf = w1 * np.exp(-0.5 * ((v - mu1) / sig1) ** 2) / sig1 + w2 * np.exp(-0.5 * ((v - mu2) / sig2) ** 2) / sig2
pmf /= pmf.sum()
...
# Mark individual components at the same scale as the total PMF
d1 = np.exp(-0.5 * ((v - mu1) / sig1) ** 2) / sig1
d2 = np.exp(-0.5 * ((v - mu2) / sig2) ** 2) / sig2
combined = w1 * d1 + w2 * d2
norm = combined.sum()
c1 = w1 * d1 / norm
c2 = w2 * d2 / norm
ax.plot(
    ...
    linewidth=1.0,
    linestyle=":",
    label=rf"Component A ($\mu={mu1:.0f}$, $\sigma={sig1:.0f}$ km/s)",
)
ax.plot(
    v,
    c2,
    ...
    linewidth=1.0,
    linestyle="-.",
    label=rf"Component B ($\mu={mu2:.0f}$, $\sigma={sig2:.0f}$ km/s)",
)
