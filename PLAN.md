...
| Coverage/recovery is *also* biased on plain-Gaussian or otherwise-unremarkable truths, not just sharp-featured ones | A broader instance of the same shrinkage mechanism, not confined to non-Gaussian truths as originally predicted. Seen twice: `compute_summary`'s kurtosis biased +1.6–2.5 even for Gaussian truth (§1.3, tail leakage into far-edge bins, amplified by kurtosis's 4th-power weighting), and the 2D solver's recovered variance biased high at the plan-suggested `n_stars~300` (§3.3, fixed by using `n_stars=2000` in the recovery tests instead of loosening tolerance). Rule of thumb: many free grid cells relative to star count → expect shrinkage bias in variance/kurtosis-like statistics; increase N or investigate mitigation, don't just widen the test tolerance (§1.3, §3.3) |
